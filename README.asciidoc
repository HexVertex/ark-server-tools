= ARK: Survival Evolved & Ascended Linux Server Tools
...
See <<Changes for ASA>> to see adjustments required to run ASA (through Wine).
...
Changes for ASA
---------------
This fork contains some minor changes to the original scripts to allow the tools to work with ASA. , including:

* Added options to arkmanager.cfg:
	- `arkserverexecPrefix` will put this in front of calls to the executable. Can set this to `"wine"` in case you'd like to use Wine to run the `exe`.
	- `serverConfigFolderOverride` value will be used to redirect the config folder locations, as opposed to the default `LinuxServer` directory
* Changed package ID for the server package `ARK: Survival Ascended Dedicated Server`.
* Changed some default config parameters to their defaults for ASA, such as ports and mapname. You should still be able to run Survival Evolved server instances through this fork (possibly even simultaniously if system resources allow it) by setting the appropriate configuration items.

Tested to be working with Wine 8.20. Alternatively to calling wine with `arkserverexecPrefix` you can set-up a binfmt binding to call windows executables (exe) natively.
Currently no adjustments have been made to facilitate modded servers, but they should work as is through the `arkopt_mods="<mod_ids>"` configuration.
