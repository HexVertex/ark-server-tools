...
* Added options to arkmanager.cfg:
	- `arkserverexecPrefix` will put this in front of calls to the executable. Can set this to `"wine"` in case you'd like to use Wine to rune the `exe`.
...
* Changed some default config parameters to their defaults for ASA, such as ports and mapname

Currently no adjustments have been made to facilitate modded servers.
